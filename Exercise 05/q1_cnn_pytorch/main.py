...
def val(net, val_dataloader):
    ...
    with torch.no_grad():
        for sample in enumerate(val_dataloader):
            ...
            total += label.size(0)
            accuracy += torch.sum(torch.argmax(logits, dim=1) == label)
    return accuracy, total


def zeft(net, test_dataloader, classes):
    net.eval()
    ...
    for sample in enumerate(test_dataloader):
        ...
        #Calculate test accuracy also confusion matrix
            ...
            predicted_labels = np.append(predicted_labels, torch.argmax(logits, dim=1).cpu().detach().numpy())
            true_labels = np.append(true_labels, label.cpu().detach().numpy())
    accuracy = 100 * (np.sum(predicted_labels == true_labels) / true_labels.size)
    df_cm = confusion_matrix(true_labels, predicted_labels)
    df_cm = pd.DataFrame(100 * df_cm / np.sum(df_cm))
...
if __name__ == '__main__':
    ...
    train_dataset_transform = transforms.Compose([transforms.Resize(size=64),
                                                  transforms.ToTensor(),
                                                  transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                       std=[0.229, 0.224, 0.225])])
    val_dataset_transform = transforms.Compose([transforms.Resize(size=64),
                                                  transforms.ToTensor(),
                                                  transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                       std=[0.229, 0.224, 0.225])])
    test_dataset_transform = transforms.Compose([transforms.Resize(size=64),
                                                  transforms.ToTensor(),
                                                  transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                       std=[0.229, 0.224, 0.225])])
    # fetch training data
    ...
    for epoch in range(num_epochs):
        ...
        for sample in enumerate(train_dataloader):
            ...
            train_acc += torch.sum(torch.argmax(logits, dim=1) == label)
    ...
    acc, df_cm = zeft(model, test_dataloader, classes)
